...
class DataEngine {
    ...
    static getRawDataCacheTtlMs(source = DEFAULT_DATA_SOURCE) {
        const normalizedSource = this.normalizeDataSource(source);
    ...
    static _getTimeZoneParts(timestamp, timeZone) {
    ...
    static _getTimeZoneOffsetMinutes(timestamp, timeZone) {
    ...
    static _getPbeCacheExpiryTimestamp(fetchedAt) {
    ...
    static _normalizeRawDataSnapshot(snapshot, source = DEFAULT_DATA_SOURCE) {
    ...
    static _isRawDataSnapshotFresh(snapshot, source = DEFAULT_DATA_SOURCE, now = Date.now()) {
    ...
    static async fetchAndParse(options = {}) {
        ...
        if (!rawData) {
            try {
                ...
                // Cache raw data for offline fallback and TTL-based reuse.
