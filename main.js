...
function getPreparedSearchContextKey(dataFingerprint, params) {
    return getCacheKey(dataFingerprint, params);
}
...
async function readCache(key, dataFingerprint) {
    ...
    if (
        ...
        memoryEntry.searchVersion === SEARCH_CACHE_VERSION
    ) {
...
ipcMain.handle(IPC_CHANNELS.GET_SEARCH_ESTIMATE, async (event, params) => {
    ...
    const { preparedContext } = getPreparedSearchContext(dataCache, normalizedParams);
    const estimate = Engine.getCombinationCount(dataCache, normalizedParams, preparedContext);
    searchEstimateMemoryCache.set(estimateKey, estimate);
    return estimate;
});
...
ipcMain.handle(IPC_CHANNELS.SEARCH_BOARDS, async (event, params) => {
    ...
    const { preparedContext } = getPreparedSearchContext(searchDataCache, normalizedParams);
