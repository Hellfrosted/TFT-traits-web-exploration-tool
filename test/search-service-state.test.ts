...
describe('search service state helpers', () => {
    ...
    it('derives progress payloads for renderer IPC', () => {
        assert.deepEqual(createWorkerProgressPayload(12, { pct: 30, checked: 15, total: 50 }), {
            searchId: 12,
            pct: 30,
            checked: 15,
            total: 50
    ...
    it('creates success, failure, and cancelled worker responses', () => {
        assert.deepEqual(createWorkerDoneResponse({ success: true, results: [{ units: ['A'] }] }, 3), {
            success: true,
            cancelled: false,
            fromCache: false,
            results: [{ units: ['A'] }],
        ...
        assert.deepEqual(createWorkerDoneResponse({ success: false, error: 'boom' }, 4), {
            success: false,
            cancelled: false,
            fromCache: false,
            results: [],
        ...
        assert.deepEqual(createWorkerErrorResponse(new Error('broken'), 5, true), createCancelledSearchResponse(5));
    });

    it('creates deterministic worker exit responses', () => {
        assert.deepEqual(createWorkerExitResponse(0, 9), {
            success: false,
            cancelled: false,
            fromCache: false,
            results: [],
        ...
        assert.deepEqual(createWorkerExitResponse(2, 10), {
            success: false,
            cancelled: false,
            fromCache: false,
            results: [],
