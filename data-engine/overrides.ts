...
module.exports = {
    ...
    _normalizeConditionalEffects(conditionalEffects) {
        ...
        return conditionalEffects
            .map((effect) => {
                const traitContributions = Object.entries(effect?.traitContributions || {}).reduce(
                    (result, [trait, count]) => {
                        const normalizedTrait = String(trait || '').trim();
                        const numericCount = Math.trunc(Number(count));
        ...
        return conditionalProfiles
            .map((profile) => {
                ...
                const normalizedRemoveTraits = (profile?.removeTraits || [])
                    .map((trait) => String(trait || '').trim())
                    .filter(Boolean);
                const normalizedTraitContributions = Object.entries(profile?.traitContributions || {}).reduce(
                    (result, [trait, count]) => {
                        const normalizedTrait = String(trait || '').trim();
                        const numericCount = Math.trunc(Number(count));
                        if (!normalizedTrait || !Number.isFinite(numericCount) || numericCount <= 0) {
    ...
    _buildConditionalProfiles(baseTraits, conditionalProfiles = []) {
        return this._normalizeConditionalProfiles(conditionalProfiles)
            .map((profile) => {
            ...
            let selectionStates = [
                {
                    ...
                    labelParts: [],
                    role: baseRole,
                    slotCost: Number.isFinite(Number(unitOverride?.slotCost))
                        ? Number(unitOverride.slotCost)
                        : undefined,
                    addTraits: [],
                    removeTraits: [],
                    traitContributions: {},
                ...
                selectionStates.forEach((state) => {
                    options.forEach((option, optionIndex) => {
                        nextStates.push({
                            idParts: [
                                ...state.idParts,
                                String(option.id || `${group.id || `group-${groupIndex}`}-${optionIndex + 1}`)
                            ],
                            labelParts: [
                                ...state.labelParts,
                                String(option.label || option.id || `Option ${optionIndex + 1}`)
                            ],
                            role: option.role || state.role,
                            slotCost: Number.isFinite(Number(option.slotCost))
                                ? Number(option.slotCost)
                                : state.slotCost,
