...
(function initializeMultiSelectComponent() {
    ...
    function setupMultiSelect(containerId, options, isUnit = true) {
        ...
        if (!pillsContainer || !input || !dropdown) {
            console.warn(`MultiSelect shell is incomplete: ${containerId}`);
            return createUnavailableController({
                getValues: () => [...preservedValues],
                setValues: (values) => previousController?.setValues?.(values),
                resolvePills: (hashMap) => previousController?.resolvePills?.(hashMap),
                destroy: () => previousController?.destroy?.()
            });
        }
        ...
        function addPill(value, { emit = true } = {}) {
            ...
            remove.addEventListener('click', () => {
                selectedValues = selectedValues.filter((selectedValue) => selectedValue !== normalizedValue);
                pill.remove();
                emitChange();
        ...
        function renderDropdown(filter = '') {
            ...
            filteredOptions = options.filter((option) => {
                const name = getOptionValue(option);
                const label = getOptionLabel(option);
                const meta = getOptionDropdownMeta(option);
            ...
            filteredOptions.forEach((option, index) => {
                ...
                const iconUrl = getOptionIconUrl(option);
                const dropdownMeta = getOptionDropdownMeta(option);
        ...
        const api = {
            ...
            destroy: () => {
                controller.abort();
                if (container._multiSelectController === api) {
